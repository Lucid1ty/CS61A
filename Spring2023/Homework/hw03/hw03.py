...
def num_eights(n):
    ...
    True
    """
    return 0 if n == 0 else num_eights(n // 10) + (1 if n % 10 == 8 else 0)
    # Solution:
    # if n % 10 == 8:
...
def count_coins(change):
    ...
    def helper(coin, change):
        if change == 0:
            return 1
        elif change < 0 or coin is None:
            return 0
        else:
            return helper(coin, change - coin) + helper(next_smaller_coin(coin), change)
