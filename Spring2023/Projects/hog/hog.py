...
def perfect_square(score):
    """Return whether a number is a perfect square."""
    return score ** 0.5 == int(score ** 0.5)


def next_perfect_square(score):
    """Return the next perfect square after a number."""
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for x in range(goal):
        for y in range(goal):
            if strategy(x, y) != strategy(x, y + 1):
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    def average(*args):
        total = [original_function(*args) for _ in range(total_samples)]
        return sum(total) / total_samples
    return average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    averages = [make_averaged(roll_dice, total_samples)(x, dice) for x in range(1, 11)]
...
def final_strategy(score, opponent_score):
    """If rolling 0, 1 or 2 dice can win the game, roll 0, 1 or 2."""
    # BEGIN PROBLEM 12
    if score + tail_points(opponent_score) >= GOAL:
        return 0
    elif score + roll_dice(1) >= GOAL:
        return 1
    elif score + roll_dice(2) >= GOAL:
        return 2
    else:
        return 6
    # END PROBLEM 12
